const mixxoPost = (function (mod) {
  ...
  let defaultGravatarCDN = "//www.gravatar.com/avatar/"; //'//gravatar.loli.net/avatar/'
  ...
  mod.init = function (config) {
    ...
    config.flexDirection === undefined
      ? (flexDirection = defaultFlexDirection)
      : ((flexDirection = config.flexDirection), (defaultInfoWidth = "")); // input flex aixs : css
    frameWidth =
      config.frameWidth === undefined ? defaultFrameWidth : config.frameWidth;
    gravatarCDN =
      config.gravatarCDN === undefined
        ? defaultGravatarCDN
        : config.gravatarCDN; // CDN
  ...
  let addmixxoDOM = () => {
    ...
    document
      .querySelector("#mixxopost")
      .insertAdjacentHTML("afterbegin", layout);
  };
  ...
  let mixxopost = () => {
    ...
    // 唤醒免费版云引擎，用于收取新留言，推送到邮箱
    let openCloudEngine = () => {
      AV.Cloud.run("run").then(
        function (data) {
          // 调用成功，得到成功的应答 data
    ...
    let logInAdmin = function (nick, admin, password) {
      if (nick === adminNick) {
        AV.User.logIn(admin, password).then(
          loggedInuser => {
    ...
    let createPOST = function (
      ...
      todo.save().then(
        function (index) {
          ...
          $("#mp-post-lists").insertAdjacentHTML(
            "afterbegin",
            mpPostBodyString(
              Date.parse(index.createdAt),
              i.usernick,
              addAdminTag(i.usernick),
              index.id,
              index.id,
              "",
              i.comments,
    ...
    let createFloor = function (
      ...
      floor.set("url", encodeURI(decodeURI(window.location.pathname)));
    ...
    const replyButtonCtr = function () {
      ...
      $("#mp-post-lists").addEventListener("click", event => {
        ...
        if (event.target.className === "mpr-reply") {
          ...
          let re = at !== "" ? "@" + at : "";
          MP.parentNode.classList.add("transform-hidden");
        ...
        if (event.target.className === "mpr-delete") {
          ...
          MP.parentNode.classList.add("transform-hidden");
          $(".mp-delete-confirm").insertAdjacentHTML(
            "beforeend",
            event.target.parentNode.parentNode.parentNode.parentNode.innerHTML
              .replace(/mpr\-reply/g, "mpr-cancel")
              .replace(/reply/g, "cancel")
          );
          ...
          $(".mp-delete-confirm").addEventListener("click", event => {
            ...
            if (c === "mpr-delete") {
              ...
              let mpLi = document.getElementById(
                event.target.parentNode.getAttribute("parent-id")
              ).parentNode;
      ...
      $("#mp-overlay").addEventListener("click", event => {
        $("#mixxopost").parentNode.classList.remove("transform-hidden");
    ...
    let mpDOM = function () {
      ...
      getComments(encodeURI(decodeURI(window.location.pathname))); // 获取评论
      ...
      $(".mixxo-user-info").addEventListener("input", event => {
        ...
        target.value.length > 0
          ? mixxoUserInfoClass(true, target.id)
          : mixxoUserInfoClass(false, target.id);
      });
      ...
      $("#mp-comment").addEventListener("input", event => {
        event.target.parentNode.children[1].children[0].innerText =
          maxCommentLength - event.target.innerText.length;
        if (!/\r\n|\n|\r/.test(event.target.innerText)) {
      ...
      $("#mp-success-tips").addEventListener(
        "animationend",
